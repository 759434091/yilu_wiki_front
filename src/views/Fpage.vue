          ...
          <ul>
            <li class="reply" v-for="item in replies" :key="item.yfrId">
                ...
                <p class="info">
                    ...
                    <span > {{ item.yfrId }} 楼<span v-show="item.yfrReplyTo"> ( 回复{{item.yfrReplyTo}}楼 ) </span> </span>
                    <span class="datetime">最后编辑于 {{ item.yfrUpdateTime }}</span>
                    <span style="float: right;">
                      <span v-if="item.yuId==yuInfo.ywUser.yuId">
                        <a class="edit" @click="isVisible=true;mTitle='修改内容';operateId=item.yfrId;editor.txt.html(item.yfrContent);submitStatus=3">编辑</a>
                        <span>/</span>
                        <a class="delete" @click="delReply(item.yfrId)">删除</a>
                        <span>/</span>
                      </span>
                      <a class="oreply" @click="isVisible=true;operateId=item.yfrId;mTitle='回复';editor.txt.clear();submitStatus=1;">回复本层</a>
                    </span>
                </p>
            </li>
          </ul>
